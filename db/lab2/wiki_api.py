...
class WikiApi:
    ...
    def print_sections(self, sections, summary, url, categoryName, pageId, level=0):
        for s in sections:
            self.print_sections(s.sections, summary, url, categoryName, pageId, level + 1)
            item = ItemWikiApi(categoryName, summary, url, s.title, s.text, pageId)
    ...
    def print_categorymembers(self, categorymembers, level=0, max_level=0):
        for c in categorymembers.values():
            ...
            if self.filename.find("Category:") == -1:
                ...
                self.print_sections(page_py.sections, page_py.summary, page_py.fullurl, c.title, str(c.pageid))
